...
### 관리자 기능 완성
- `src/app/api/admin/users/route.ts` — Supabase service_role 키로 사용자 CRUD API
- `src/app/admin/page.tsx` — 앱 내 관리자 페이지 (가족 계정 추가/삭제)
- `src/components/layout/Header.tsx` — 관리자 이메일 로그인 시 관리자 버튼 노출
- Supabase secret key 노출 사고 → rolAPIkey.txt 삭제, .gitignore 추가, 키 재발급

### UI 버그 수정
- 경비/일정 추가 모달: 하단 네비게이션에 가려지는 문제 수정 (z-[60], max-h-[85vh])
- 모달 내 날짜 입력 잘림 수정 (overflow-hidden 컨테이너 감싸기)
- 경비/일정 추가 모달 입력 필드 세로 배치로 변경 (좌우 잘림 완전 해결)
- 여행 추가/수정 페이지 출발일·귀국일 세로 배치로 변경

### 경비 수정/삭제 기능
- `src/lib/db.ts` — updateExpense 함수 추가
- 경비 항목 탭 → 수정 모달 오픈 (카테고리, 날짜, 금액, 내용 수정)
- 수정 모달 내 삭제 버튼 추가

### 경비 통계 개선
- 전체/국내/해외 필터 추가 (연도 필터와 동시 적용)
- 예정 여행 경비 제외 (완료된 여행만 통계에 포함)
- 평균 경비 기준: 여행 횟수 → 1박 평균으로 변경
- 연도별 차트 하단 버튼 금액도 타입 필터 반영

### 권한 정책
- 현재: 로그인한 모든 사용자가 동일하게 CRUD 가능
- 관리자 페이지(계정 관리)만 관리자 이메일로 제한
- 추후 필요 시 일반 사용자 수정/삭제 제한 예정
